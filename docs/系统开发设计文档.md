...
PanSou 是一款高性能的网盘资源搜索 API 服务，采用纯 Go 实现，支持 TG 频道和插件搜索。系统采用异步插件架构，具备二级缓存机制和并发控制能力，在 MacBook Pro 8GB 上能够支持 500 用户并发访问。源码编译部署，简单高效，跨平台支持。

### 1.2 核心特性

- **纯 Go 实现**: 单一语言技术栈，无需容器化，源码编译即可运行
...
深度搜索（Deep Search）是 pioz 插件的核心功能，通过 Next.js Server Action 机制直接获取带有网盘链接的搜索结果，无需二次跳转访问详情页。

### 12.2 技术原理

#### Next.js Server Action 机制

pioz 网站基于 Next.js 框架开发，其深度搜索功能通过 Server Action 实现：

```mermaid
sequenceDiagram
    participant Client as PanSou服务
    participant Server as pioz网站
    participant Action as Server Action
    
    Client->>Server: POST / (with next-action header)
    Server->>Action: 路由到对应Action
    Action->>Action: 执行深度搜索逻辑
    Action->>Action: 聚合多源数据
    Action-->>Client: 返回带链接的结果
```
...
next-action: 406ba109bf31d3d81924420f284be8a50f5694e5fc
Body: ["太奶奶"]
```
...
```
0:{"a":"$@1","f":"","b":"h1Jz1vYhB7fynD3D8YGOp","q":"","i":false}
...
```go
// 深度搜索主函数
func (p *PiozAsyncPlugin) performDeepSearchViaServerAction(
    client *http.Client, 
    keyword string,
) ([]model.SearchResult, error)

// 响应解析函数
func (p *PiozAsyncPlugin) parseDeepSearchServerActionResponse(
    body []byte,
) []model.SearchResult

// 结果增强函数
func (p *PiozAsyncPlugin) enhanceDeepSearchResults(
    client *http.Client, 
...
flowchart TD
    A[开始搜索] --> B{缓存命中?}
    B -->|是| C[返回缓存结果]
    B -->|否| D[深度搜索 Server Action]
    D --> E{成功?}
    E -->|是| F[增强结果]
    E -->|否| G[deep-search API]
    G --> H{成功?}
    H -->|是| I[详情页增强]
    H -->|否| J[HTML搜索页]
    J --> K{成功?}
...
type DeepSearchResponse struct {
    ...
    Results []struct {
        ID        string `json:"id"`         // "1771487613375_0"
        Title     string `json:"title"`      // 资源标题
        URL       string `json:"url"`        // 网盘链接
        CloudType string `json:"cloud_type"` // baidu/quark
        Password  string `json:"password"`   // 提取密码
        Datetime  string `json:"datetime"`   // 发布时间
    } `json:"results"`
}
```
...
if err == nil && len(results) > 0 {
    // 深度搜索成功
    return p.enhanceDeepSearchResults(client, results)
}

// 降级到 API 搜索
results, err = p.performDeepSearch(client, keyword)
if err == nil && len(results) > 0 {
    return p.enhanceWithDetails(client, results)
}
...
1. **Action ID 时效性**：网站更新后 Action ID 可能失效，需要重新提取
2. **请求头设置**：必须设置正确的 `Accept` 和 `next-action` 头
3. **ID 类型**：响应中的 `id` 字段是字符串类型，不是整数
4. **缓存策略**：搜索结果缓存 30 分钟，避免重复请求
